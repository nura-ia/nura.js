...
export type NTelemetryWildcardHandler = (payload: NTelemetryPayload & { event: NTelemetryEvent }) => void
...
export function createTelemetry(): NTelemetry {
  ...
  const getHandlers = (event: string | '*'): Set<InternalHandler> => {
    if (!map.has(event)) map.set(event, new Set())
    return map.get(event) as Set<InternalHandler>
  }
  return {
    on(event, handler) { getHandlers(event).add(handler as InternalHandler) },
    off(event, handler) { getHandlers(event).delete(handler as InternalHandler) },
    emit(event, payload) {
      const normalized = payload ?? {}
      for (const handler of getHandlers(event)) handler(normalized)
      for (const handler of getHandlers('*')) handler({ event, ...normalized })
    },
  }
}
